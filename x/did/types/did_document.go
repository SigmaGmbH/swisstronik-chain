...
func (doc DIDDocument) Validate(allowedNamespaces []string) error {
	return validation.ValidateStruct(&doc,
		validation.Field(&doc.Id, validation.Required, IsDID()),
		validation.Field(&doc.Controller, IsUniqueStrList(), validation.Each(IsDID())),
		...
		validation.Field(&doc.Authentication,
			IsUniqueStrList(), validation.Each(IsDIDUrl(Empty, Empty, Required), HasPrefix(doc.Id)),
		),
