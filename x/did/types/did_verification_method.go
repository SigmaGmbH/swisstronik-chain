...
func VerifySignature(vm VerificationMethod, message []byte, signature []byte) error {
	...
	case Ed25519VerificationKey2020Type:
		...
		keyBytes := GetEd25519VerificationKey2020(multibaseBytes)
		verificationError = VerifyED25519Signature(keyBytes, message, signature)
	...
	case JSONWebKey2020Type:
		...
		case *rsa.PublicKey:
			verificationError = VerifyRSASignature(*pubKey, message, signature)
		case *ecdsa.PublicKey:
			verificationError = VerifyECDSASignature(*pubKey, message, signature)
		case ed25519.PublicKey:
			verificationError = VerifyED25519Signature(pubKey, message, signature)
		default:
	...
	case Ed25519VerificationKey2018Type:
		...
		verificationError = VerifyED25519Signature(publicKeyBytes, message, signature)
...
func VerificationMethodListToMapByFragment(vms []*VerificationMethod) map[string]VerificationMethod {
	...
	for _, vm := range vms {
		_, _, _, fragment := MustSplitDIDUrl(vm.Id)
		result[fragment] = *vm
	}
...
func (vm *VerificationMethod) ReplaceDIDs(old, new string) {
	...
	// Id
	vm.Id = ReplaceDIDInDIDURL(vm.Id, old, new)
}
