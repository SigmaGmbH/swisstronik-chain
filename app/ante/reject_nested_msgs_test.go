...
func TestRejectNestedMessagesDecorator(t *testing.T) {
	...
	decorator := ante.NewRejectNestedMessageDecorator(
	...
	testCases := []struct {
		...
		{
			"enabled msg MsgHandleTx - blocked msg not wrapped in MsgExec",
			[]sdk.Msg{
		...
		},
		{
			...
			[]sdk.Msg{
				newMsgGrant(
					testAddresses[0],
					testAddresses[1],
					authz.NewGenericAuthorization(sdk.MsgTypeURL(&evmtypes.MsgHandleTx{})),
					&distantFuture,
				),
			},
			false,
...
func (suite *AnteTestSuite) TestRejectMsgsInAuthz() {
	...
	testcases := []struct {
		...
		{
			name:         "a MsgGrant with MsgHandleTx typeURL on the authorization field is blocked",
			msgs:         []sdk.Msg{newMsgGrant(sdk.MsgTypeURL(&evmtypes.MsgHandleTx{}))},
			expectedCode: sdkerrors.ErrUnauthorized.ABCICode(),
		},
		{
			name:         "a MsgGrant with MsgHandleTx typeURL on the authorization field included",
			msgs:         []sdk.Msg{newMsgGrant(sdk.MsgTypeURL(&evmtypes.MsgHandleTx{}))},
			expectedCode: sdkerrors.ErrUnauthorized.ABCICode(),
		},
		{
			name: "a MsgExec with nested messages (valid: MsgSend and invalid: MsgHandleTx) is blocked",
			msgs: []sdk.Msg{
				newMsgExec(
					testAddresses[1],
					[]sdk.Msg{
