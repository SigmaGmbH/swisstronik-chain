...
def wasmer22_supported(os, cpu, build_type):
    if os == "Windows (mingw)":
        if cpu == "x86_64" and build_type == "shared":
            return UNDER_CONSTRUCTION + "sgx_wrapper.dll"
        else:
            return UNSUPPORTED
    if os == "macOS" and build_type == "static":
        return UNSUPPORTED
    if os == "macOS" and build_type == "shared":
        return SUPPORTED + "libsgx_wrapper.dylib"
    if os == "Linux (musl)":
        if build_type == "static":
            if cpu == "x86_64":
                return SUPPORTED + "libsgx_wrapper_muslc.x86_64.a"
            elif cpu == "aarch64":
                return SUPPORTED + "libsgx_wrapper_muslc.aarch64.a"
    ...
    if os == "Linux (glibc)":
        ...
        if build_type == "shared":
            if cpu == "x86_64":
                return SUPPORTED + "libsgx_wrapper.x86_64.so"
            elif cpu == "aarch64":
                return SUPPORTED + "libsgx_wrapper.aarch64.so"
    return UNKNOWN
...
def get_note(os, cpu, build_type):
    ...
    if os == "Linux (glibc)" and cpu == "x86_64" and build_type == "static":
        return "Would link libsgx_wrapper statically but glibc dynamically as static glibc linking is not recommended. Potentially interesting for Osmosis."
...
print("<!-- AUTO GENERATED BY libsgx_wrapper_builds.py START -->")
...
print(get_links())
print("<!-- AUTO GENERATED BY libsgx_wrapper_builds.py END -->")
