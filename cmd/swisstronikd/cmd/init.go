...
func InitCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			default:
				chainID = fmt.Sprintf("swisstronik_9000-%v", tmrand.Str(6))
			}

			// Get bip39 mnemonic
			var mnemonic string
			doRecover, _ := cmd.Flags().GetBool(genutilcli.FlagRecover)
			if doRecover {
			...
			stakingBondDenom, _ := cmd.Flags().GetString(genutilcli.FlagStakingBondDenom)
	...
	cmd.Flags().String(genutilcli.FlagStakingBondDenom, "", "genesis file staking bond denomination, if left blank default value is 'stake'")
